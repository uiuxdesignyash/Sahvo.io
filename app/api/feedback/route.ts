...
export async function POST(request: Request) {
  try {
    if (!SCRIPT_URL || !SCRIPT_SECRET) {
      console.error('[feedback] Missing env vars:', { url: !!SCRIPT_URL, secret: !!SCRIPT_SECRET });
    ...
    const shape = validateRequestShape(request, 4096);
    if (!shape.ok) return shape.response;

    const body = await request.json();
    const { rating, segment, q2, q3, comment, name, email, company, mountedAt, consentVersion } = body;
    ...
    const trimmedEmail = typeof email === 'string' ? email.trim().slice(0, 254) : '';
    ...
    try {
      ...
      console.error('[feedback] Apps Script error:', res.status, res.statusText, responseText.slice(0, 500));
      return NextResponse.json(
      ...
      if (fetchErr.name === 'AbortError') {
        console.error('[feedback] Apps Script timeout after 60s');
        return NextResponse.json(
          { ok: false, error: 'Request timed out. Please try again.' },
          { status: 500 },
        );
      }
      console.error('[feedback] Fetch failed:', fetchErr?.message, fetchErr?.code);
      throw fetchErr;
    }
  } catch (error: any) {
    console.error('[feedback] Unhandled error:', error?.message, error?.stack);
    return NextResponse.json(
      { ok: false, error: 'Something went wrong. Please try again.' },
      { status: 500 },
    );
